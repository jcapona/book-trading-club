...
module.exports = {
	...
	deleteBook: function(req, res) {
		var book_id = req.params.all().id
		Book.destroy({id: book_id}).exec(function(err, book) {
			if (err)
				return res.negotiate(err);
			Prop.destroy({book_id: book_id}).exec(function(err, book) {
				if (err)
					return res.negotiate(err);
				Interact.destroy({b_id: book_id}).exec(function(err, interact) {
					if (err)
						return res.negotiate(err);
					return res.send({});
				});
